#!/usr/bin/env python3
"""
Sentient Wallet Security AI Agent - Lightweight Vercel Deployment
FastAPI serverless application for wallet security analysis (no heavy dependencies)
"""
...
# Import only the essential wallet security agent (no visualization)
from wallet_security_agent import WalletSecurityAgent

# Initialize FastAPI app
app = FastAPI(
    title="Sentient Wallet Security AI Agent",
    description="AI-powered wallet security analysis (Lightweight API)",
    version="1.0.0"
)
...
async def analyze_wallet(request: WalletAnalysisRequest):
    ...
    try:
        # Lazy init if global init failed
        global wallet_agent
        ...
        # Generate basic stats (no heavy visualization)
...
def generate_basic_stats(transactions):
    """Generate basic transaction statistics without heavy dependencies"""
    if not transactions:
        return {}
